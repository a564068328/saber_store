...
public class ColorArcProgressBar extends View{
    ...
    private String longDegreeColor = "#111111"; //黑色
    private String shortDegreeColor = "#111111";
    private String bgArcColor = "#ff6E717A";  //背景圆弧为灰色
...
//        diameter = 3 * getScreenWidth() / 5;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isNeedContent) {
            canvas.drawText(String.format("%.0f", curValues), centerX, centerY + textSize / 2, vTextPaint);
        }
        if (isNeedUnit) {
            canvas.drawText(hintString, centerX, centerY + 3 * textSize/2 , hintPaint);
        }
        if (isNeedTitle) {
            canvas.drawText(titleString, centerX, centerY - 2 * textSize , curSpeedPaint);
        }
