...
public class Msecond extends BaseFragement implements OnClickListener {
...
//		handler = new mHandler(this);
	...
	public void connectService() {
		intent = new Intent(act, playerService.class);
		intent.putExtra("path", Environment.getExternalStorageDirectory()
				+ "/com.icloud.listenbook/" + "忏悔歌.mp3");
		act.startService(intent);
		conn = new myBindService();
		act.bindService(intent, conn, act.BIND_AUTO_CREATE);
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_adult:
			if (UserInfo.instance().isLogin()) {
				Intent intent = new Intent(act, ThirdAdult.class);
				intent.putExtra("isAdult", true);
				LoadingTool.launchActivity(act, ThirdAdult.class, intent);
			} else {
	...
	public void downMP3() {
		// 判断手机内存是否够下载文件
		if (DownFileUtils.canDownload(act, Url)) {
