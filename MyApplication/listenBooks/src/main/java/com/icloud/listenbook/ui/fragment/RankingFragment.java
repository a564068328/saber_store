...
public class RankingFragment extends BaseFragement implements
	...
	public void init() {
		...
		lineLM = new LinearLayoutManager(getActivity());
		lineLM.setOrientation(LinearLayoutManager.VERTICAL);
		adapter = new RankAdapter(getActivity());
	}
	...
	public void getRankDatas() {
		...
		if (item.size() > 0)
			return;
		if (DataUpManage.isUp(getActivity(), "getRankDatas" + page)) {
	...
	public void onResponse(JSONObject response) {
		try {
			...
			if (res == 0) {
				...
				DataUpManage.save(getActivity(), "getRankDatas" + rankid);
			} else {
				getRankDatasCache();
			}
		} catch (Exception e) {
